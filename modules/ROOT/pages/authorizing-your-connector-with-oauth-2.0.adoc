...
If a Cloud Connector or Custom Module is to take advantage of the OAuth facilities the DevKit provides, the class level annotation `@OAuth2` needs to be present. Some parameters are required; others are optional and use default values that are suitable for most of the scenarios.
...
|verifierRegex |a Java regular expression used to extract the verifier from the service provider response after the resource owner authorizes the consumer |OPTIONAL |code=([^&]+) +
...
|callbackPath |If the service provider accepts only a known redirect URL for this parameter, use the path inside your domain (denoted by the `fullDomain` environment variable). If left empty and the service provider accepts any URL as a redirect URL for this parameter, a random path is used. |OPTIONAL |_ random path _
|===
...
Consult Mule ESB's xref:3.3@mule-runtime::https-transport-reference.adoc[*HTTPS Transport*] documentation for more detail on how to configure an HTTPS connector.