...
Access xref:authentication-methods.adoc[OAuth V1] or xref:authentication-methods.adoc[OAuth V2] Authentication for further details on how to connect to an API which uses OAuth.
...
|Basic Authentication ^|Yes |xref:authentication-methods.adoc[Basic Auth]
|OAuth 1.0 ^|Yes |xref:authentication-methods.adoc[OAuth V1]
|OAuth 2.0 ^|Yes |xref:authentication-methods.adoc[OAuth V2]
|SAML ^|No* |
|Kerberos ^|No* |
|NTLM ^|No* |
|LDAP ^|No* |
...
* *NEXT*: Continue to xref:authentication-methods.adoc[Defining Attributes Operations and Data Model] for your Connector.