...
You must also arrange access to the API.  The process is specific to each API provider and the typical steps for this process are described in xref:setting-up-your-api-access.adoc[Setting up your API access].
...
[TIP]
For more information on configuring an *HTTPS connector,* consult the xref:3.5@mule-runtime::https-transport-reference.adoc[HTTPS Transport Reference].
