...
The connector passes all calls to the Dropbox API through this method to add the authentication headers specified by the OAuth V1 standard. Because this is specific to the use of Jersey client, a detailed walkthrough of this method and how it fits into the client class is not presented here.
...
[NOTE]
For more information on configuring an *HTTPS connector,* see the xref:3.6@mule-runtime::https-transport-reference.adoc[HTTPS Transport Reference] and xref:3.6@mule-runtime::tls-configuration.adoc#examples-for-https[Examples for HTTPS].
