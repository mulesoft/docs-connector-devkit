...
Also, update XML schemas and namespaces. See the xref:3.5@mule-runtime::vm-transport-reference.adoc[VM endpoint documentation] for details.
...
. Manually get the access token per Facebook's https://developers.facebook.com/tools/explorer[Graph API Explorer].
...
----
+
. Add `accessTokenId="accessTokenId"` to the operations in automation-test-flows. This value is resolved by the ObjectStore.
+
[source,xml,linenums]
----
...
After you complete your test suite, you can xref:creating-reference-documentation.adoc[complete documentation and samples] for your connector.