...
This document assumes you are familiar with RESTful APIs, and with the DevKit connector architecture described in xref:index.adoc[Anypoint Connector DevKit].
...
image::restcall-connector-arch.png[]
...
* The OAuth2 annotations are used on the relevant methods and properties, as described in xref:oauth-v2.adoc[OAuth V2]
...
For details on developing connector tests, see xref:developing-devkit-connector-tests.adoc[Developing DevKit Connector Tests].
====
...
* Refine the appearance of the connector dialog boxes and XML element through more annotations, as described in xref:defining-connector-attributes.adoc[Defining Connector Attributes].
