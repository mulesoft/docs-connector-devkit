...
If a Cloud Connector or Custom Module is to take advantage of the OAuth facilities the DevKit provides, the class level annotation `@OAuth` needs to be present. Some parameters are required; others are optional and take default values that are suitable for most of the scenarios.
...
|verifierRegex |s Java regular expression used to extract the verifier from the service provider response after the resource owner authorizes the consumer |OPTIONAL |oauth_verifier=([^&]+)
