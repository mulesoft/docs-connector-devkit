...
This document assumes you are familiar with the xref:index.adoc[Anypoint Connector DevKit].
...
. Install https://www.mulesoft.com/platform/studio[Anypoint Studio]

Refer to xref:anypoint-connector-development.adoc[Setting Up Your Development Environment] for detailed instructions.
...
Refer to xref:setting-up-your-api-access.adoc[Setting Up Your API Access] for detailed instructions.
...
|xref:basic-auth.adoc[Basic Auth] |The simplest mode of authentication, often used with SOAP web services.
...
|xref:creating-a-connector-using-a-java-client-library.adoc[Java Client Library] |If an official or well-known third party library is available, this is generally the simplest connector style to implement, and is recommended. Most client libraries are actually wrappers for SOAP or RESTful APIs.
|xref:creating-a-connector-for-a-soap-service-via-cxf-client.adoc[SOAP API] |The de facto standard library for building and consuming SOAP Web services, this style handles client logic and Java object mapping to XML.
|xref:creating-a-connector-for-a-restful-api-using-restcall-annotations.adoc[RESTful API] |Anypoint DevKit provides a built-in client for RESTful APIs. Useful for cleanly designed RESTful APIs.
|===

For more details, refer to xref:defining-attributes-operations-and-data-model.adoc[Defining Attributes and Operations and Data Model].
...
. Create an instance of xref:3.7@mule-runtime::functional-testing.adoc[FunctionalTestCase] that runs the test. For DevKit testing, you run Mule inside a JUnit test case. Each test case you build will extend the class xref:3.7@mule-runtime::functional-testing.adoc[FunctionalTestCase] (a subclass of JUnit). The test case manages the lifecycle of a Mule runtime instance and calls flow to carry out the test.
...
Refer to xref:creating-reference-documentation.adoc[Creating a Reference Documentation] for further details.
...
* *NEXT*: Understand xref:setting-up-your-dev-environment.adoc[Setting Up Your Development Environment].