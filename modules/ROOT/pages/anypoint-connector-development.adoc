...
. Install the xref:setting-up-your-dev-environment.adoc#installing-the-devkit-plugin-in-studio[Anypoint DevKit Plugin].
...
. Follow the process in xref:creating-an-anypoint-connector-project.adoc[Creating an Anypoint Connector Project] to start a connector development project in your development environment.
...
For more details, refer to xref:defining-attributes-operations-and-data-model.adoc[Defining Attributes Operations and Data Model].
...
. Create an instance of xref:connector-testing-framework.adoc[FunctionalTestCase] to run the test. For DevKit testing, run Mule inside a JUnit test case. Each test case you build extends the class xref:connector-testing-framework.adoc[FunctionalTestCase] (a subclass of JUnit). The test case manages the lifecycle of a Mule runtime instance and calls flows to carry out the test.
...
See xref:setting-up-your-dev-environment.adoc[Setting Up Your Development Environment].