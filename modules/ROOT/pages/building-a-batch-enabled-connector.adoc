...
* the synchronization of data sets between business applications

Some of the already existing connectors, such as xref:3.5@mule-runtime::salesforce-connector.adoc[Salesforce] and http://www.mulesoft.com/cloud-connectors/netsuite-integration-connector[NetSuite], have been supporting batch operations for years. However, with the introduction of the new batch module, developers no longer need to parse the response of the connector to determine which of the records failed. Instead, this is handled automatically by `<batch:commit>`.

This document explains how you can leverage your own connector to take advantage of this feature introduced in the xref:3.5@mule-runtime::batch-processing.adoc[Batch Processing] documentation.

image::6-package.png[]

== Assumptions

In this document it is assumed that you are familiar with how xref:3.5@mule-runtime::batch-processing.adoc[Batch Processing] works. It is further assumed that you have already xref:creating-an-anypoint-connector-project.adoc[created a connector project] and xref:defining-attributes-operations-and-data-model.adoc[defined the operations] to be used.
...
* Generate the xref:creating-reference-documentation.adoc[Reference Documentation].