...
+
image::installing-and-testing-your-connector-in-studio-69499.png["Install window to add your connector"]
+
...
+
image::36barnconnector.png["Simple flow with Barn Connector"]
+
...
+
image::config1.png["Config panel for Barn Connector"]
+
...
+
image::config.png["Config panel for Barn Connector with an arrow pointing to the green plus icon"]
+
. In the *My Property*, *Username*, and *Password* fields, enter any string as the value, then click *OK*. +
 +
image::global-elements.png["Global Elements Properties window for Barn Connector"]
+
...
+
image::browser.png["Browser with the response of the Mule flow"]
