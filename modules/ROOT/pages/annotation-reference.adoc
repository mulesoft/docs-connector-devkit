...
*http://mulesoft.github.io/mule-devkit/3.5.3/apidocs/index.html[DevKit 3.5 API]*
...
----
|`@ExpressionLanguage` a|Defines a class that exports its functionality as a Mule Expression Language.
...
This annotation marks a method inside a http://mulesoft.github.io/mule-devkit/3.5.3/apidocs/index.html[Module] as callable from within a Mule flow and capable of generating Mule events. Each marked method has an `org.mule.api.source.MessageSource` generated. The method must receive a SourceCallback as one of its arguments. It does not matter which parameter it is as long it is there.
...
----
|`@InvocationHeaders` a|Passes invocation headers. This can be a single header, a comma-seperated list of header names, an asterisk "*" to denote all headers, or a comma-seperated list of wildcard expressions. By default, if a named header is not present, an exception is thrown. However, if the header name is defined with the "?" post fix, it is marked as optional.
...
For more information, see xref:basic-auth.adoc[Basic Authentication]
...
For information, see xref:3.5@mule-runtime::datasense.adoc[DataSense], xref:adding-datasense.adoc[Adding DataSense], and xref:adding-datasense-query-language.adoc[Adding DataSense Query Language]
...
----
|`@Initialise` a|Mark a method to be initialized during a method's `org.mule.lifecycle`. initializable phase. *Note:* `initialise` is a reserved word that cannot be used as the method's name.
...
@Connect(strategy=ConnectStrategy.SINGLE_INSTANCE)
    public void connect(@ConnectionKey String username, @Password String password)
        throws ConnectionException { … }
----
|`@Default` a|Specifies a default value to a `@Configurable` field or a `@Processor` or `@Source` parameter.
...
throws IOException;
----
|`@RestHttpClient` a|An annotation to mark the HttpClient module uses. This way, you avoid creating multiple clients and have the opportunity to perform your own calls or to configure the HttpClient to fulfill special needs:
