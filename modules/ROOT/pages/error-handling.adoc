...
This page describes these various error handling techniques. For information on setting retry policies, which control how a connector behaves when its connection fails, see xref:3.2@mule-runtime::configuring-reconnection-strategies.adoc[Configuring Reconnection Strategies].
...
[TIP]
For details on configuring exception strategies, see: xref:3.2@mule-runtime::exception-strategy-configuration-reference.adoc[Exception Strategy Configuration Reference]. +
 +
For examples of the most common exception strategy configurations, see xref:3.2@mule-runtime::exception-strategy-most-common-use-cases.adoc[Exception Strategy Most Common Use Cases]
...
For more information on routers, see xref:3.2@mule-runtime::using-message-routers.adoc[Using Message Routers].
...
For more information on filters, see xref:3.2@mule-runtime::using-filters.adoc[Using Filters].

== Error Handling with Transactions

If you are using xref:3.2@mule-runtime::transaction-management.adoc[transactions], you can use the `<commit-transaction>` and `<rollback-transaction>` elements to specify whether a transaction gets committed or rolled back on the basis of the name of the exception that is caught.
