...
* To work through the example, you have created a Maven project for a connector as described in  xref:creating-a-connector-project.adoc[Creating a Connector Project]  as a starting point.
...
See xref:preparing-api-access-or-a-sandbox-environment.adoc[Preparing API Access or a Sandbox Environment] for steps that may be required to gain access to other APIs, including how to get access to the WSDL file.
...
. Import or re-import your Maven project to your IDE, as described in Writing Extensions in IntelliJ or "Importing a Maven Project into Eclipse/Mule Studio" in xref:creating-a-connector-project.adoc[Creating a Connector Project].
...
The different authentication methods are discussed in xref:authentication-and-connection-management.adoc[Authentication and Connection Management]; find your authentication method and refer to the examples for guidance on how to add authentication handling in the proxy client.
====
...
DevKit enforces JavaDoc documentation of your methods. One of the things you must add is an XML sample of the inputs required by each connector method. xref:creating-devkit-connector-documentation.adoc[Learn more] about JavaDoc annotations for DevKit.
...
See xref:creating-devkit-connector-documentation.adoc[Creating DevKit Connector Documentation] for full information on filling in the JavaDoc for your connector.
...
See xref:installing-and-testing-your-connector.adoc[Installing and Testing Your Connector] for the steps to follow to build your connector and import it into Studio.
...
* Return to the xref:devkit-shortcut-to-success.adoc[DevKit Shortcut to Success]