...
This is a means to managing user access to the service through the connector. For more information on session token placement inside the body see <<enriching-soap-body-with-session-token,Enriching SOAP Body with Session Token>>.
