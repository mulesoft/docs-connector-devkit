...
image::create-connector-architecture.png[]

== The Example Service: SunSetRiseService

The web service for the demo is the SunSetRise web service. See xref:3.7@creating-a-connector-for-a-soap-service-via-cxf-client.adoc#appendix-sunsetriseservice-wsdl[SunSetRise WSDL].
...
CXF includes the wsdl2java utility that can generate Java stub client code to call any method on the service, and marshall and unmarshal request parameters and responses as Java objects for further processing. This generated stub client becomes the core of your connector.
...
See xref:setting-up-your-api-access.adoc[Setting Up Your API Access] for steps that may be required to gain access to other APIs, including how to get access to the WSDL file.
...
. Import or re-import your Maven project to your IDE, as described in "Importing a Maven Project into Eclipse/Mule Studio" in xref:creating-an-anypoint-connector-project.adoc[Creating an Anypoint Connector Project].
...
DevKit enforces JavaDoc documentation of your methods. One of the things you must add is an XML sample of the inputs required by each connector method. xref:creating-reference-documentation.adoc[Learn more about JavaDoc annotations for DevKit].
...
See xref:creating-reference-documentation.adoc[Creating DevKit Connector Documentation]
...
* Check out other xref:anypoint-connector-examples.adoc[examples]
