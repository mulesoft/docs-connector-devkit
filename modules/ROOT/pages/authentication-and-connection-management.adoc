...
image::5-package.png[title="5-package.png"]

== Assumptions

This document assumes you have xref:creating-a-connector-project.adoc[created a connector project], xref:installing-and-testing-your-connector.adoc[installed and tested] it in Mule Studio, and are ready to implement authentication on your connector.
...
See xref:implementing-oauth-2.0-authentication.adoc[Implementing OAuth 2.0 Authentication] for details on how to implement OAuth 2 authentication in your connector.
...
See xref:implementing-oauth-1.0-authentication.adoc[Implementing OAuth 1.0 Authentication] for details on how to implement OAuth 1 authentication in your connector.
...
xref:implementing-connection-management.adoc[Implementing Connection Management] describes applying the connection management to your connector by applying annotations to @Connector class methods. The example describes using the annotations with basic username/password authentication, but the same methods apply for other methods as well.

Connection management can also be applied in connectors that use no authentication at all. The xref:connector-to-soap-service-via-cxf-client-example.adoc[Connector to SOAP Service via CXF Client Example] illustrates implementing connection management in a connector with no authentication.
...
* Read an overview of xref:authentication-methods.adoc[authentication methods].
