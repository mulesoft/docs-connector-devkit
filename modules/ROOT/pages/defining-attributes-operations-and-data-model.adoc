...
More details can be found in xref:3.7@defining-connector-attributes.adoc[Defining Connector Attributes].

== Static Versus Dynamic Data Models

The underlying data model for the API you connect to often drives design decisions about the best way to represent data going to and from your connector. The goal in general is to choose a representation that fits the underlying data, while still supporting the necessary metadata to work with xref:3.6@mule-runtime::datasense.adoc[DataSense].
...
More details can be found in Static Data Model & Dynamic Data Model Examples in this document about xref:3.7@adding-datasense.adoc[Adding DataSense].
...
* *NEXT:* Continue to xref:3.7@defining-connector-attributes.adoc[Defining Connector Attributes].
* You can also jump into your specific use case, checking the xref:3.7@anypoint-connector-examples.adoc[examples].
