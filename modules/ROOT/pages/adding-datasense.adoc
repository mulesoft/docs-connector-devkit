...
This document assumes that you are familiar with the connector architecture presented in xref:index.adoc[Anypoint Connector DevKit], and that you are familiar with xref:studio::datasense.adoc[DataSense] from an end-user's perspective.
...
[NOTE]
The *fundamental advantage of DataSense* is the ability to extract metadata from the data model exposed by an application through the connector. Learn more about xref:studio::datasense.adoc[DataSense].
