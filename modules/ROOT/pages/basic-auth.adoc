...
OAuth-based authentication in DevKit provides functionality comparable to the Basic Authentication, if you use the OAuth-related annotations. See xref:oauth-v1.adoc[OAuth V1] and xref:oauth-v2.adoc[OAuth V2] for details.
====
...
The username parameter is annotated with `@ConnectionKey`. If pools are enabled (see xref:basic-auth.adoc[Connection Pooling] below), Mule keeps a pool of simultaneous connections which are used as needed to make calls. The `@ConnectionKey` annotation marks that this field is used as the key for this particular connection within the connection pool, so once a connection for this username has been created and added to the pool, it will be reused rather than recreated for each request.
[NOTE]
====
...
[WARNING]
For clients where no authentication is used, you must still supply a `@ConnectionKey` field. An invented username is one possibility; see xref:creating-a-connector-for-a-soap-service-via-cxf-client.adoc[Creating a Connector for a SOAP Service Via CXF Client] for such an implementation.
...
* See the xref:creating-a-connector-for-a-soap-service-via-cxf-client.adoc[Creating a Connector for a SOAP Service Via CXF Client] example to see the Basic Authentication annotations in a fully working example.