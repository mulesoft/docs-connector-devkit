...
image::6-package.png["A flowchart showing the steps in a development process"]
...
Create a new class in your connector project that extends the `DefaultQueryVisitor` class.
...
image::qb2.png["A property editor with text fields labeled Connection, Notes, Config Reference, Operation, Language, Query Builder, and Query Text"]
...
image::query-builder.png["A window with Query Builder configuration"]
...
image::qb3.png["A property editor with the Native Query Language selected"]
...
image::dm.png["DataMapper configuration window"]
