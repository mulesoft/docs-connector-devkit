...
* Familiarity with Anypoint connector architecture as described in xref:anypoint-connector-concepts.adoc[Anypoint Connector Concepts]
* Familiarity with the @Connector class methods of implementing authentication, as described in xref:authentication-and-connection-management.adoc[Authentication and Connection Management]
* If you are developing a client library-based connector, start by creating a new connector project, as described in xref:creating-a-connector-project.adoc[Creating a Connector Project].
...
* Because the connector does not use DevKit's OAuth support, it is possible to use DevKit's xref:implementing-connection-management.adoc[connection management framework].
====
...
For non-OAuth authentication, add @Configurable properties on the @Connector class, as described in xref:defining-configurable-connector-attributes.adoc[Defining Configurable Connector Attributes], and take advantage of the connection management framework, as described in xref:authentication-and-connection-management.adoc[Authentication and Connection Management].
...
* xref:authentication-and-connection-management.adoc[Authentication and Connection Management] for an overview
* xref:implementing-connection-management.adoc[Implementing Connection Management] for non-OAuth authentication schemes
* xref:implementing-oauth-1.0-authentication.adoc[Implementing OAuth 1.0 Authentication] and xref:implementing-oauth-2.0-authentication.adoc[Implementing OAuth 2.0 Authentication] for OAuth-enabled schemes.
...
If you are merely reviewing the different connector implementation types, you can return to xref:connector-attributes-operations-and-data-model.adoc[Connector Attributes Operations and Data Model] to review connector implementations that communicate directly with SOAP and RESTful web services without using a pre-built client library.
...
* Return to the xref:devkit-shortcut-to-success.adoc[DevKit Shortcut to Success] to continue the development process described there
...
* Build out the documentation examples to show more samples, based on information in xref:creating-devkit-connector-documentation.adoc[Creating DevKit Connector Documentation]