...
* You are familiar with the connector architecture presented in xref:anypoint-connector-concepts.adoc[Anypoint Connector Concepts].
* You are familiar with xref:3.4@mule-runtime::mule-datasense.adoc[Mule DataSense] from a user perspective.
...
`List<MetaDataKey>` keys store https://github.com/mulesoft/mule-common/blob/support/3.x/src/main/java/org/mule/common/metadata/DefaultMetaDataKey.java[DefaultMetadataKey] instances. DefaultMetadataKey's constructor identifies metadata keys by a `String id`:
